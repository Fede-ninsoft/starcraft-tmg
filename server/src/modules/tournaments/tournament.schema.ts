...
export const tournamentCommandSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('RESULT'), matchId: id, vp: z.tuple([z.number().int().min(0).max(999), z.number().int().min(0).max(999)]), end: z.enum(['NORMAL', 'TIME', 'CONCESSION', 'NO_SHOW', 'GAME_LOSS']), winner: z.union([z.literal(0), z.literal(1)]).nullable(), rosterIds: z.tuple([id.nullable(), id.nullable()]), reason: z.string().max(1000) }),
  z.object({ type: z.literal('DISPUTE'), matchId: id, reason }),
  z.object({ type: z.literal('RESOLVE'), matchId: id, vp: z.tuple([z.number().int().min(0).max(999), z.number().int().min(0).max(999)]), end: z.enum(['NORMAL', 'TIME', 'CONCESSION', 'NO_SHOW', 'GAME_LOSS']), winner: z.union([z.literal(0), z.literal(1)]).nullable(), reason }),
