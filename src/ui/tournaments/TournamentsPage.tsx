  ...
  return <section className="tournaments">
    ...
    {!id && <>
      ...
      <section className="t-directory-filters" aria-label={text('Filtrar torneos', 'Filter tournaments')}>
        ...
        <p className="t-directory-note"><TournamentIcon name="info" /><span>{text('Los torneos publicados o en curso se finalizan automáticamente más de 48 horas después de su fecha de fin. Hasta entonces permanecen en Torneos Activos, salvo que se finalicen o cancelen antes.', 'Published or in-progress events are completed automatically more than 48 hours after their scheduled end. Until then they remain in Active Tournaments, unless completed or cancelled earlier.')}</span></p>
      </section>
      ...
      {!directoryLoading && (['current', 'past'] as const).map((period) => {
        ...
        return <section key={period} className="t-directory-group" data-period={period} aria-labelledby={`tournaments-${period}`}>
          <div className="t-directory-group-heading"><span className="t-directory-group-icon"><TournamentIcon name={period === 'current' ? 'swords' : 'flag'} /></span><div><h2 id={`tournaments-${period}`}>{period === 'current' ? text('Torneos Activos', 'Active Tournaments') : text('Pasados', 'Past')}</h2><p>{period === 'current' ? text('Torneos futuros y en curso, aún no finalizados ni cancelados.', 'Upcoming and in-progress events that have not been completed or cancelled.') : text('Torneos finalizados o cancelados.', 'Completed or cancelled events.')}</p></div></div>
          ...
          {!visible.length && <p className="t-directory-empty">{period === 'current' ? text('No hay torneos activos que coincidan con la búsqueda.', 'No active tournaments match your search.') : text('No hay torneos pasados que coincidan con la búsqueda.', 'No past events match your search.')}</p>}
        </section>;
      })}
