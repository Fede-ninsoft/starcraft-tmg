...
export function TournamentsPage() {
  ...
  return <section className="tournaments">
    ...
    {!id && <>
      ...
      <div className="t-period-filters" role="group" aria-label={text('Filtrar torneos', 'Filter tournaments')}>{(['all', 'current', 'future', 'past'] as const).map((v, i) => <button key={v} aria-pressed={period === v} onClick={() => setPeriod(v)}><TournamentIcon name={v === 'current' ? 'swords' : v === 'past' ? 'flag' : 'calendar'} />{[text('Todos', 'All'), text('En curso', 'Current'), text('Futuros', 'Upcoming'), text('Pasados', 'Past')][i]}</button>)}</div><p className="t-muted">{text('En curso: listas cerradas y hasta dos días después del fin del torneo.', 'Current: rosters closed and up to two days after the event ends.')}</p>
      <div className="tournament-actions"><label>{text('Buscar en los torneos cargados', 'Search loaded tournaments')}<input value={query} onChange={(e) => setQuery(e.target.value)} /></label>{user && <label><input type="checkbox" checked={mine} onChange={(e) => setMine(e.target.checked)} /> {text('Organizados por mí', 'Hosted by me')}</label>}</div>
      <div className="tournament-grid">{entries.filter((e) => (!mine || e.ownerId === user?.id) && `${e.config.name} ${e.config.location}`.toLowerCase().includes(query.toLowerCase())).map((e) => <article key={e.id}><EventSummary config={e.config} ownerName={e.ownerName} count={e.playerCount} status={e.status} statusLabel={status(e.status)} text={text} date={(v) => new Date(v).toLocaleString(locale, { timeZone: e.config.timezone, dateStyle: 'medium', timeStyle: 'short' })} title={<a href={`${base}/${e.id}`} onClick={(ev) => { ev.preventDefault(); navigate(`${base}/${e.id}`); }}>{e.config.name}</a>} /></article>)}</div>
    ...
    {t && <>
      <EventSummary config={t.config} ownerName={t.ownerName} count={t.players.filter((p) => p.status === 'ACTIVE').length} status={t.status} statusLabel={status(t.status)} date={date} text={text} />
      ...
      {tab === 'info' && <div className="tournament-card"><p className="tournament-prose">{t.config.description}</p><dl className="t-info-grid">{([['rosterDeadlineAt', text('Plazo de listas', 'Roster deadline')], ['rostersPublicAt', text('Publicación de listas', 'Roster publication')]] as const).map(([key, label]) => <div key={key}><dt><TournamentIcon name="calendar" />{label}</dt><dd>{date(t.config[key])}</dd></div>)}</dl><p className="t-format-note"><TournamentIcon name="list" />{t.config.listFormat === 'DUAL' ? text('Una o dos listas de la misma raza; pueden tener cartas de facción distintas.', 'One or two rosters of the same race; faction cards may differ.') : text('Lista única', 'Single roster')}</p>
        ...
        {!self && user?.emailVerified && t.status === 'PUBLISHED' && now < Date.parse(t.config.startsAt) && <form className="tournament-actions" onSubmit={(e) => { e.preventDefault(); void send({ type: 'JOIN', race, token: invitation || undefined }); }}><label>{text('Tu raza', 'Your race')}<select value={race} onChange={(e) => setRace(e.target.value as typeof race)}>{['ZERG', 'TERRAN', 'PROTOSS'].map((v) => <option key={v}>{v}</option>)}</select></label>{t.config.registrationMode === 'INVITE_ONLY' && !invitation && <p>{text('Abre el enlace de invitación que te facilite el organizador.', 'Open the invitation link supplied by the organiser.')}</p>}<button disabled={busy || t.config.registrationMode === 'INVITE_ONLY' && !invitation}>{text('Inscribirme', 'Join')}</button></form>}
